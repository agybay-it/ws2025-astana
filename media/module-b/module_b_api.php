...
switch ($requestUri) {
    case '/module_b_api.php/reset.json':
        header('Content-Type: application/json');
        $_SESSION['request_count'] = 0;
        echo json_encode(['Reset' => 'Carpark seed reset done.']);
        break;
    case '/module_b_api.php/carparks.json':
        header('Content-Type: application/json');
        echo json_encode(getCarparkData($_SESSION['request_count']));
        break;

    case '/module_b_api.php/weather.json':
        header('Content-Type: application/json');
        echo json_encode(getWeatherData());
        break;

    default:
        if (strpos($requestUri, '/module_b_api.php/image.png') === 0) {
            createImageWithText($_GET['title']);
        } else if (strpos($requestUri, '/module_b_api.php/events.json') === 0) {
            header('Content-Type: application/json');
            ...
            header('Content-Type: application/json');
            $list = [
                '/module_b_api.php/reset.json' => 'Reset random seed',
                '/module_b_api.php/carparks.json' => 'Get carpark availability',
                '/module_b_api.php/weather.json' => 'Get weather data',
                '/module_b_api.php/events.json' => 'Get events data',
                '/module_b_api.php/image.png?title=Hello, World' => 'Get image with text',
            ];
